        ...
        <>
           ...
           <RxCross2 className={`${overlaycheck == false && "hidden"} , md:hidden`} onClick={overlay} />
                ...
                <ul ref={overlayref} className={`md:flex md:p-0 p-2 md:bg-white  bg-blue-400 md:static absolute top-[52px] right-0 md:w-full   ${overlaycheck == true && "h-screen w-[200px] text-xl right-[-10px] font-semibold"}   gap-4 text-lg ${overlaycheck == false ? "hidden" : "block"}` } >
                    <li onClick={()=> setoverlaycheck(false)} className='md:mt-0 mt-2' ><Link to="/">Home</Link></li>
                    <li onClick={()=> setoverlaycheck(false)} className='md:mt-0 mt-2' ><Link to="/store">Store</Link></li>
                    <li onClick={()=> setoverlaycheck(false)} className='md:mt-0 mt-2' ><Link to="/cart">Cart ({cart.length})</Link></li>
                    <li onClick={()=> setoverlaycheck(false)} className='md:mt-0 mt-2' ><Link to="/contact">About Us</Link></li>
                </ul>
            </div>
            </div>
        </>
