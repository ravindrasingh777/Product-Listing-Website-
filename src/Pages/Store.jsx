        ...
        <> 
          <div className='max-w-[1100px] relative mx-auto flex justify-between p-2 items-center'>
            <div className='w-full grid grid-cols-4'>
                <div className='border md:h-[900px] h-[1138px] sticky  top-14 left-0'>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="h-4 bg-gray-300 rounded w-full"></div>
                            ...
                            Category.map(
                                (item,index)=>{
                                  return(
                                    <li key={index} className={`p-1 border text-center ${category_slug == item.slug && "bg-blue-500 text-white"}`}><Link to={`/${item.slug}`}>{item.name}</Link></li>
                                  )
                                }
                            )
                        }
                ...
                <div className='text-center  bg-slate-600 p-[11px] flex justify-end text-lg text-white'>
                    <div className='flex justify-between items-center w-full'>
                      ...
                      <input ref={inpref} className='h-[25px] md:w-full w-[120px] rounded p-[5px] text-black outline-none' type="text" placeholder='search item' />
                      <button onClick={search} className=' h-[20px] text-[12px]  flex items-center justify-center'><i className="fa-solid fa-magnifying-glass"></i></button>
                      </div>
                   <div  className='flex gap-2'>
                   <IoGrid  className={`${Listingmode == 0 && "text-[orange]"} cursor-pointer`} onClick={()=> setListingmode(0)}  />
                   <FaListUl className={`${Listingmode == 1 && "text-[orange]"} cursor-pointer`}  onClick={()=> setListingmode(1)} />
                   </div>
                    </div>
                </div>
                <div className='gap-2 justify-between w-full flex flex-wrap'>
                  
                   {
                    ...
                    :
                    Products_response.products?.map(
                            (item)=>{
                                return(
                        
                                    <div key={item.id} className={`${Listingmode == 0 ? "md:w-[23%] w-[47%] mt-1 ml-1 p-2 border" : "w-full py-3 flex border items-center justify-start"}`}>
                                    <Link to={`/details/${item.id}`}><img className="md:w-[200px] w-[200px]" src={item.thumbnail} alt="" /></Link>
                                    <div className='flex-col w-full'>
                                    <h5 className=' text-xs'>{item.title}</h5>
                                    <div className={`${Listingmode == 0 ? "flex text-sm justify-between mt-2" : "mt-0"}`}>
                                       <h6 className='hidden md:block' >{item.brand}</h6>
                                       <h6 className='text-xs text-red-500'>${item.price}</h6>
                                    </div>
                                    <div>
                                      <button onClick={()=> carthandler({id:item.id, Image:item.thumbnail,price:item.price})} className='border-[1px] w-full rounded bg-blue-600 text-white mt-2 border-black py-1 px-2'>+ Cart</button>
                                    </div>
                                    {
                                        Listingmode == 1 &&
                                        <p className='mt-2 md:block hidden'>{item.description}</p>
                                    }
                                     {
                                        Listingmode == 1 &&
                                        <p className='text-green-600 font-bold mt-2'>{item.availabilityStatus}</p>
                                    }
                                    </div>
                                   </div>
                   ...
                   <div className='m-4 '>
                   <Pagination currentpage={page} Pagechangehandler={Pagechangehandler} limit={limit} total_records={Products_response.total}/>
                   </div>
                </div>
...
const Pagination=({limit, total_records,Pagechangehandler,currentpage})=>{
   ...
   for(let i=0; i<total_pages;i++){
     total_elem.push(
      ...
      <p
        ...
        className={`flex ${currentpage == i && "!bg-blue-600 text-white"} cursor-pointer items-center justify-center px-3 h-8 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300  hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white`}
      >
        {i + 1}
      </p>
    </li>
